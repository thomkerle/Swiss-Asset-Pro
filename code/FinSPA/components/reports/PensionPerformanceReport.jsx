    ...
    <div className="max-w-6xl px-4 md:px-8 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
         <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm">
            ...
            <div className="text-xs text-gray-400 mt-2">{t ? t('descPensionInvested') || 'Einzahlungen & Sparpläne' : 'Einzahlungen & Sparpläne'}</div>
         </div>
         ...
         <div className={`border p-6 rounded-2xl shadow-sm ${latestData.profit >= 0 ? 'bg-indigo-50 border-indigo-200 dark:bg-indigo-900/20' : 'bg-orange-50 border-orange-200 dark:bg-orange-900/20'}`}>
            <div className={`text-xs font-bold uppercase tracking-wider mb-2 ${latestData.profit >= 0 ? 'text-indigo-700 dark:text-indigo-400' : 'text-orange-700 dark:text-orange-400'}`}>{t ? t('labelTotalReturnYields') || 'Total Return (inkl. Erträge)' : 'Total Return (inkl. Erträge)'}</div>
            ...
            return (
                <div key={cls} className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 p-5 rounded-2xl shadow-sm flex flex-col">
                    ...
                    <div className="space-y-3 text-sm flex-1">
                        <div className="flex justify-between items-center"><span className="text-gray-500">{t ? t('labelInvested') || 'Investiert:' : 'Investiert:'}</span><span className="font-mono">{fCur ? fCur(stats.invested, 'CHF') : stats.invested}</span></div>
                        <div className="flex justify-between items-center"><span className="text-gray-500">{t ? t('labelYieldsInterestDiv') || 'Erträge (Zinsen/Div):' : 'Erträge (Zinsen/Div):'}</span><span className="font-mono text-indigo-500">+{fCur ? fCur(stats.yields, 'CHF') : stats.yields}</span></div>
                        <div className="flex justify-between items-center"><span className="text-gray-500">{t ? t('labelMarketValue') || 'Marktwert:' : 'Marktwert:'}</span><span className="font-mono font-bold text-slate-800 dark:text-slate-200">{fCur ? fCur(stats.actual, 'CHF') : stats.actual}</span></div>
                    </div>
                    <div className={`mt-4 pt-3 border-t border-gray-100 dark:border-slate-800 flex justify-between font-bold ${isPos ? 'text-indigo-600 dark:text-indigo-400' : 'text-orange-600 dark:text-orange-400'}`}>
                        <span>{t ? t('labelTotal') || 'Total:' : 'Total:'} {isPos ? '+' : ''}{fCur ? fCur(totalProfit, 'CHF') : totalProfit}</span>
            ...
            <div ref={chartRef} style={{ width: '100%', height: '350px' }}>
                <UniversalChart 
                    ...
                    datasets={[
                        ...
                        {
                            label: t ? t('labelTotalValueYields') || 'Total Value (inkl. Erträge)' : 'Total Value (inkl. Erträge)',
            ...
            <table className="w-full text-sm text-left">
               <thead className="text-xs text-gray-500 uppercase bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800">
                  <tr>
                     ...
                     <th className="px-6 py-4 text-right text-blue-600 dark:text-blue-400">{t ? t('labelYields') || 'Erträge' : 'Erträge'}</th>
