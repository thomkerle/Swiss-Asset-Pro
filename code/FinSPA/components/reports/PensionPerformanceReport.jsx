...
const PensionPerformanceReport = ({ data, activeAssets, isTreeVisible, setIsTreeVisible, fCur, t }) => {
  // Filtern auf alle Vorsorge Assets (2. und 3. Säule)
    ...
    return (
      <div className="max-w-6xl px-4 md:px-8 pb-12">
        ...
        <div className="bg-gray-50 dark:bg-slate-900 border-2 border-dashed border-gray-300 dark:border-slate-700 rounded-xl p-10 text-center text-gray-500">
          ...
          <p className="text-sm mt-2">Lege ein Asset mit der Anlageklasse "Pensionskasse", "3a Vorsorgekonto" oder "3a Vorsorgefonds" an.</p>
        </div>
      </div>
    );
  }
  ...
  // 2. Hilfsfunktion zur Investitionsberechnung
  ...
  // 3. Zeitreihen-Punkte für das Diagramm berechnen (Kumuliert)
  const dataPoints = [];
  ...
  // 4. Detail-Aufschlüsselung der 3 Säulen-Typen per Heute
  const currentStats = {
      pension_cash: { invested: 0, actual: 0 },
      pension_3a_cash: { invested: 0, actual: 0 },
      pension_3a_fund: { invested: 0, actual: 0 }
  };

  pensionAssets.forEach(asset => {
      const inv = calculateInvested(asset, todayStr);
      const act = getAssetValueAtDate(asset, todayStr, activeAssets);
      if (currentStats[asset.assetClass]) {
          currentStats[asset.assetClass].invested += inv;
  ...
  // 5. Simples, responsives SVG-Liniendiagramm berechnen
  ...
  return (
    <div className="max-w-6xl px-4 md:px-8 pb-12">
      <div className="mb-8 border-b border-gray-200 dark:border-slate-800 pb-6">
         <ReportHeader title="Vorsorge & Pensionskasse" subtitle="Einzahlungen im Vergleich zum echten Marktwert (2. & 3. Säule)" isTreeVisible={isTreeVisible} setIsTreeVisible={setIsTreeVisible} />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
         ...
         <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 p-6 rounded-2xl shadow-sm">
            <div className="text-gray-500 text-xs font-bold uppercase tracking-wider mb-2">Total Marktwert</div>
         ...
         <div className={`border p-6 rounded-2xl shadow-sm ${isPositive ? 'bg-green-50 border-green-200 dark:bg-green-900/20 dark:border-green-800/50' : 'bg-red-50 border-red-200 dark:bg-red-900/20 dark:border-red-800/50'}`}>
            <div className={`text-xs font-bold uppercase tracking-wider mb-2 ${isPositive ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400'}`}>Gesamt Gewinn / Verlust</div>
         ...
         <div className={`border p-6 rounded-2xl shadow-sm ${isPositive ? 'bg-green-50 border-green-200 dark:bg-green-900/20 dark:border-green-800/50' : 'bg-red-50 border-red-200 dark:bg-red-900/20 dark:border-red-800/50'}`}>
            <div className={`text-xs font-bold uppercase tracking-wider mb-2 ${isPositive ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400'}`}>Gesamt Rendite (ROI)</div>
            <div className={`text-2xl font-black ${isPositive ? 'text-green-700 dark:text-green-400' : 'text-red-700 dark:text-red-400'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {pensionClasses.map(cls => {
            ...
            if (stats.invested === 0 && stats.actual === 0) return null; // Keine Assets dieser Klasse
            
            const profit = stats.actual - stats.invested;
            const roi = stats.invested > 0 ? (profit / stats.invested) * 100 : 0;
            const isPos = profit >= 0;
            
            const titleMap = {
                'pension_cash': 'Pensionskasse (2. Säule)',
                'pension_3a_cash': '3a Vorsorgekonto (Cash)',
                'pension_3a_fund': '3a Vorsorgefonds (Depot)'
            };
            const iconMap = {
                'pension_cash': 'Building',
                'pension_3a_cash': 'Lock',
                'pension_3a_fund': 'TrendingUp'
            };
            ...
            return (
                <div key={cls} className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 p-5 rounded-2xl shadow-sm flex flex-col">
                    ...
                    <div className={`mt-4 pt-3 border-t border-gray-100 dark:border-slate-800 flex justify-between font-bold ${isPos ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                        <span>{isPos ? '+' : ''}{fCur ? fCur(profit, 'CHF') : profit}</span>
      ...
      {dataPoints.length > 1 && (
         <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm mb-8">
            <h3 className="font-bold text-lg mb-6 flex items-center gap-2"><Icon name="TrendingUp" className="text-blue-500" /> Kumulierte Wertentwicklung</h3>
      ...
      <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
         <div className="px-6 py-4 border-b border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-slate-800/50 font-bold text-gray-700 dark:text-gray-300">
             Historische Datenpunkte (Gesamt)
         </div>
