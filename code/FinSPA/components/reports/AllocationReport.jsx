...
const AllocationReport = ({ data, dateRange, isTreeVisible, setIsTreeVisible, fCur, t }) => {
  ...
  return (
    <div className="max-w-6xl px-4 md:px-8 pb-12">
      <ReportHeader 
        title={t('repAlloc')} 
        subtitle={`${t('reportDate')} ${dateRange.to}`} 
        isTreeVisible={isTreeVisible} 
        setIsTreeVisible={setIsTreeVisible} 
      />
      <div className="p-8 flex justify-center bg-gray-50 dark:bg-slate-900 border dark:border-slate-800 rounded-xl">
        <PieChartSVG data={allocData} fCur={fCur} />
      </div>
    </div>
  );
};
