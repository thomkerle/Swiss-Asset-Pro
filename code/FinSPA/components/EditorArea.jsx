...
// NEU: Globale Übersetzungs-Hilfsfunktion für sauberen Code
...
const Sparkline = ({ dataSeries, dateSeries, title }) => {
    ...
    const handleMouseMove = (e) => {
        ...
        const widthEff = width - paddingX * 2;
        const pct = (mouseX - paddingX) / widthEff;
        let idx = Math.round(pct * (plotData.length - 1));
        if (idx < 0) idx = 0;
        if (idx > plotData.length - 1) idx = plotData.length - 1;
        setHoveredIdx(idx);
        
        const coords = getCoords(plotData[idx], idx);
...
const EditorArea = ({ data, viewMode, activeReport, selectedNode, setSelectedNode, isTreeVisible, setIsTreeVisible, showArchived, dateRange, setDateRange, modalObj, setModalObj, updateTreeData, fCur, t }) => {
  ...
  if (activeReport) {
    const getReportConfig = (reportId) => {
        const configs = {
            'overview': { title: safeT(t, 'repOverviewTitle', 'Vermögensübersicht'), subtitle: safeT(t, 'repOverviewSub', 'Zusammenfassung der finanziellen Situation'), iconName: 'PieChart', iconColor: 'text-blue-600 dark:text-blue-400', iconBg: 'bg-blue-100 dark:bg-blue-900/30' },
            'allocation': { title: safeT(t, 'repAllocTitle', 'Asset Allokation'), subtitle: safeT(t, 'repAllocSub', 'Verteilung über Anlageklassen'), iconName: 'Layers', iconColor: 'text-purple-600 dark:text-purple-400', iconBg: 'bg-purple-100 dark:bg-purple-900/30' },
            'liquidity': { title: safeT(t, 'repLiqTitle', 'Liquiditätsplanung'), subtitle: safeT(t, 'repLiqSub', 'Verfügbares Kapital'), iconName: 'Droplet', iconColor: 'text-cyan-600 dark:text-cyan-400', iconBg: 'bg-cyan-100 dark:bg-cyan-900/30' },
            'history': { title: safeT(t, 'repHistTitle', 'Historische Entwicklung'), subtitle: safeT(t, 'repHistSub', 'Wertverlauf über die Zeit'), iconName: 'TrendingUp', iconColor: 'text-emerald-600 dark:text-emerald-400', iconBg: 'bg-emerald-100 dark:bg-emerald-900/30' },
            'tax': { title: safeT(t, 'repTaxTitle', 'Steuer-Report'), subtitle: safeT(t, 'repTaxSub', 'Steuerrelevante Daten'), iconName: 'FileText', iconColor: 'text-rose-600 dark:text-rose-400', iconBg: 'bg-rose-100 dark:bg-rose-900/30' },
            'categoryFlow': { title: safeT(t, 'repCatFlowTitle', 'Kategorie-Flow'), subtitle: safeT(t, 'repCatFlowSub', 'Ein- und Ausgänge nach Kategorien'), iconName: 'GitMerge', iconColor: 'text-amber-600 dark:text-amber-400', iconBg: 'bg-amber-100 dark:bg-amber-900/30' },
            'waterfall': { title: safeT(t, 'repWaterTitle', 'Wasserfall-Analyse'), subtitle: safeT(t, 'repWaterSub', 'Veränderungsfaktoren'), iconName: 'BarChart2', iconColor: 'text-indigo-600 dark:text-indigo-400', iconBg: 'bg-indigo-100 dark:bg-indigo-900/30' },
            'passive': { title: safeT(t, 'repPassiveTitle', 'Passives Einkommen'), subtitle: safeT(t, 'repPassiveSub', 'Dividenden & Zinsen'), iconName: 'DollarSign', iconColor: 'text-green-600 dark:text-green-400', iconBg: 'bg-green-100 dark:bg-green-900/30' },
            'topFlow': { title: safeT(t, 'repTopFlowTitle', 'Top Transaktionen'), subtitle: safeT(t, 'repTopFlowSub', 'Grösste Bewegungen'), iconName: 'ArrowUpRight', iconColor: 'text-orange-600 dark:text-orange-400', iconBg: 'bg-orange-100 dark:bg-orange-900/30' },
            'bookingAnalysis': { title: safeT(t, 'repBookAnaTitle', 'Buchungsanalyse'), subtitle: safeT(t, 'repBookAnaSub', 'Auswertung der Transaktionen'), iconName: 'Activity', iconColor: 'text-fuchsia-600 dark:text-fuchsia-400', iconBg: 'bg-fuchsia-100 dark:bg-fuchsia-900/30' },
            'future': { title: safeT(t, 'repFutureTitle', 'Zukunftsprognose'), subtitle: safeT(t, 'repFutureSub', 'Projektion der Entwicklung'), iconName: 'FastForward', iconColor: 'text-violet-600 dark:text-violet-400', iconBg: 'bg-violet-100 dark:bg-violet-900/30' },
            'scenarios': { title: safeT(t, 'repScenTitle', 'Szenarien'), subtitle: safeT(t, 'repScenSub', 'Auswirkungen von Ereignissen'), iconName: 'Shuffle', iconColor: 'text-pink-600 dark:text-pink-400', iconBg: 'bg-pink-100 dark:bg-pink-900/30' },
            'pension3a': { title: safeT(t, 'repPenTitle', 'Vorsorge Performance'), subtitle: safeT(t, 'repPenSub', 'Säule 3a & PK'), iconName: 'Shield', iconColor: 'text-teal-600 dark:text-teal-400', iconBg: 'bg-teal-100 dark:bg-teal-900/30' },
            'securities': { title: safeT(t, 'repSecTitle', 'Wertschriften Performance'), subtitle: safeT(t, 'repSecSub', 'Aktien, Fonds & Krypto'), iconName: 'TrendingUp', iconColor: 'text-blue-600 dark:text-blue-400', iconBg: 'bg-blue-100 dark:bg-blue-900/30' },
            'dividendCalendar': { title: safeT(t, 'repDivCalTitle', 'Dividendenkalender'), subtitle: safeT(t, 'repDivCalSub', 'Zukünftige Erträge'), iconName: 'Calendar', iconColor: 'text-yellow-600 dark:text-yellow-400', iconBg: 'bg-yellow-100 dark:bg-yellow-900/30' }
        };
        return configs[reportId] || { title: 'Report', subtitle: '', iconName: 'FileText', iconColor: 'text-gray-600 dark:text-gray-400', iconBg: 'bg-gray-100 dark:bg-gray-800' };
    };
      ...
      <div className="p-8 h-full bg-white dark:bg-slate-950 overflow-auto finspa-scrollbar">
        ...
        >
            <div className="flex items-center gap-3 bg-white/60 dark:bg-slate-900/60 backdrop-blur-md border border-gray-200 dark:border-slate-700 px-4 py-2.5 rounded-xl text-sm shadow-sm transition-all hover:shadow-md">
                <Icon name="Calendar" className="text-blue-500 dark:text-blue-400" size={18}/>
  ...
  if (selectedNode) {
    ...
    if (selectedNode.type === 'asset') {
      ...
      const handleDeleteEntry = (itemToDelete, e) => {
          ...
          if (typeof window !== 'undefined' && window.showToast) window.showToast(safeT(t, 'msgDeleted', "Eintrag gelöscht"), "success");
      };
            ...
            <div className="print-hide bg-gray-50 dark:bg-slate-800 p-4 border-b flex gap-3 flex-wrap items-center">
              ...
              <div className="ml-auto flex items-center gap-2">
                ...
                {isSecurities && (
                    ...
                    >
                        <Icon name="RefreshCw" size={16}/> {safeT(t, 'btnSyncMarketValues', 'Marktwerte synchronisieren')}
                    </button>
                )}
              </div>
            </div>
                        ...
                        <React.Fragment key={`group-${monthYear}`}>
                          ...
                          {items.map(item => {
                            ...
                            if (isSecurities && item.type === 'Wertanpassung') {
                                categoryDisplay = `${safeT(t, 'typeReval', 'Wertanpassung')}: ${item.price ? `${item.price} ${adjustedNode.currency}` : safeT(t, 'systemManual', 'Manuelle Anpassung')}`;
                            }
